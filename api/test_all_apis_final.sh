...
if [ -n "$TOKEN" ]; then
    ...
    if [ -n "$NOTIFICATION_ID" ]; then
        ...
        if [ "$http_code" != "200" ] && [ "$http_code" != "204" ]; then
            ...
            echo "成功: PATCH /api/notifications/$NOTIFICATION_ID/read (状态码: $http_code)"
        fi
        echo ""
    ...
    http_code=$response
    echo "状态码: $http_code"
    # HTTP 204 No Content is a valid success response for this endpoint
    if [ "$http_code" != "200" ] && [ "$http_code" != "204" ]; then
        ...
        echo "成功: PATCH /api/notifications/read-all (状态码: $http_code)"
    fi
    echo ""
fi
...
if [ -n "$TOKEN" ]; then
    ...
    if [ -n "$GROUP_ID" ]; then
        ...
        # 测试解散群组
        echo "测试: DELETE /api/groups/$GROUP_ID"
        response=$(curl -s -o response_body.txt -w "%{http_code}" -X DELETE "$BASE_URL/api/groups/$GROUP_ID" \
          -H "Authorization: Bearer $TOKEN")
        http_code=$response
        echo "状态码: $http_code"
        if [ "$http_code" != "200" ]; then
            echo "错误: DELETE /api/groups/$GROUP_ID 返回状态码 $http_code"
            echo "响应体: $(cat response_body.txt)"
            errors+=("DELETE /api/groups/$GROUP_ID - 状态码: $http_code, 响应: $(cat response_body.txt)")
        else
            echo "成功: DELETE /api/groups/$GROUP_ID"
        fi
        echo ""
    fi
fi
...
# 清理临时文件
rm -f response_body.txt response_body2.txt response_body_join.txt